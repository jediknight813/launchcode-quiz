...
const GradeQuizQuestion = ({index, question}) => {
    ...
    <div className=" w-full h-auto pt-5 pb-5 rounded-md flex flex-col shadow-sm bg-slate-800/50 items-center">
        ...
        <p className="pr-4 pl-4 text-sm mb-4">{question['question']}</p>
        {question['user_answer'] === question['correct_answer'] &&
            <h2 className=" text-green-500 md:text-lg">Your Answer: {question.user_answer}</h2>
        }
        {question['user_answer'] !== question['correct_answer'] &&
            <h2 className=" text-red-500 md:text-lg">Your Answer: {question.user_answer}</h2>
        }
        <h3 className="text-sm md:text-lg">Correct Answer: {question.correct_answer}</h3>
    </div>
    )
}
